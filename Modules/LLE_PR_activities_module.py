...
def compute_weight_matrix(*args):
    ...
    Each row i has its Gaussian mean shifted along a line from -HSM to +HSM
    ...
    Accepts either 5 positional args (p, N, HSM, gain, rng_weights), or 4
    (N, HSM, gain, rng_weights) in which case p defaults to 1.0 (fully
    ...
    Args:
        ...
        N: number of neurons
        HSM: mean-shift half-range for the per-row Gaussian means
    ...
    """
    if len(args) == 5:
        p, N, HSM, gain, rng_weights = args
    elif len(args) == 4:
        p = 1.0
        N, HSM, gain, rng_weights = args
    else:
        raise TypeError(
    ...
    # spaced across [-HSM, HSM] -- this is what makes the matrix
