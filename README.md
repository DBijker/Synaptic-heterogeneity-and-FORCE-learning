Welcome to my Github repository where I made all my code available for the public. This code have been accumulated throughout the year for my thesis project called: "Effects of Synaptic Heterogeneity on Dynamics and Learning Performance in Recurrent Neural Networks".

The code still needs to be cleaned up, which I hope to do in the coming weeks. But it is roughly divided into 4 categories:

1. Code for activity plots (including eigenvalue spectra, and total weight distributions) which include:
- "VSCode script for Activity plots"
- "simulations_Activities"
2. Code for maximal lyapunov exponent calculations which include:
- "module_tanh_LLE"
- "LLEs for different gains and W and Init seeds"
- "module"
- "simulations_LLEs"
3. Code for participation ratio calculations which include:
- "simulations_PRs"
- "module"
- "PRs for different gains and W and Init seeds"
4. Code for RLS-FORCE learning with internal weight updates which include:
- "FORCE RLS implementation for RNN"
- "FORCE_module"
- "simulations_FORCE"
- "simulations_FORCE_HSMs_Gains_different_seeds"
- "simulations_HSMs_gains_FORCE"
- "random_search_optimization_HSMs_FORCE"
